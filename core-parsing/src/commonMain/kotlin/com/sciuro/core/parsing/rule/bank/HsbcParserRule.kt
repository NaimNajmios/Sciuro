...
class HsbcParserRule(
    private val aggregatorPackages: Set<String> = emptySet()
) : ParserRule {
    override fun matches(event: RawEvent): Boolean {
        return event.sourcePackageOrAddress == "my.com.hsbc.hsbcmalaysia" ||
               event.title.contains("HSBC", ignoreCase = true) ||
               matchesAggregatorForward(event, aggregatorPackages, listOf("HSBC".lowercase()))
    }
