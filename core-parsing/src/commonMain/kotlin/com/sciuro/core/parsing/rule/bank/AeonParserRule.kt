...
class AeonParserRule(
    private val aggregatorPackages: Set<String> = emptySet()
) : ParserRule {
    override fun matches(event: RawEvent): Boolean {
        return event.sourcePackageOrAddress == "my.com.aeonbank" ||
               event.title.contains("AEON", ignoreCase = true) ||
               matchesAggregatorForward(event, aggregatorPackages, listOf("AEON".lowercase()))
    }
