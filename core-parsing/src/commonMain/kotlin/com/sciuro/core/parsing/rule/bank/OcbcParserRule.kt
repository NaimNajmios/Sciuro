...
class OcbcParserRule(
    private val aggregatorPackages: Set<String> = emptySet()
) : ParserRule {
    override fun matches(event: RawEvent): Boolean {
        return event.sourcePackageOrAddress == "com.ocbc.my.mobilebanking" ||
               event.title.contains("OCBC", ignoreCase = true) ||
               matchesAggregatorForward(event, aggregatorPackages, listOf("OCBC".lowercase()))
    }
