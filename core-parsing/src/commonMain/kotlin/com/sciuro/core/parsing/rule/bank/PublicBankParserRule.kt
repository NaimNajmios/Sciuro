...
class PublicBankParserRule(
    private val aggregatorPackages: Set<String> = emptySet()
) : ParserRule {
    override fun matches(event: RawEvent): Boolean {
        return event.sourcePackageOrAddress == "com.pbengage.pubank" ||
               event.title.contains("Public Bank", ignoreCase = true) ||
               matchesAggregatorForward(event, aggregatorPackages, listOf("Public Bank".lowercase()))
    }
