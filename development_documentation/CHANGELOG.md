...
## [Unreleased]
### Added
- **Phase F6 — Debt Payment Link Table**: Added `debt_payment_link` table (migration `7.sqm`) with `UNIQUE(debt_id, transaction_id)` constraint for structural idempotency. Created `DebtPaymentLinkRepository` and rewrote `DebtEngine` to use link-based matching — each payment is linked once, balances are recalculated from the link table rather than raw transactions. Prevents double-counting when `processDebtPayments()` is re-invoked.
### Changed
