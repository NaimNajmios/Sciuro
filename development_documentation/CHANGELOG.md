...
## [Unreleased]
### Added
- **Phase F5 — Investment PriceProvider + Valuation Engine**: Added `PriceProvider` interface with `ManualPriceProvider` (user-set prices via `SettingsProvider`, default). Migration `6.sqm` adds `unit_type` column to `investment_record`. Created `InvestmentValuationEngine` computing `unitsHeld × currentPrice` with fallback to `averageBuyPrice`, publishing `InvestmentPriceRefreshed`. `InvestmentEngine` now publishes `InvestmentTransactionRecorded` on unit changes. `Investment` model gained `unitType` field.
### Changed
