...
## [Unreleased]
### Added
- **Phase F3 — Rule Learner, CategoryResolver, BudgetLimitSuggester**: Added `merchant_category_rule` and `recipient_rule` tables (migration `5.sqm`) for persistent rule memory. Created `RuleLearner` subscribing to `DomainEventBus` (`TransactionCategorized`/`TransactionRecategorized`) — normalizes merchant keys, upserts rules, and publishes `MerchantRuleLearned`. Created `CategoryResolver` replacing the inline `guessCategoryId()` in the orchestrator with rule-first lookup (learned → static heuristic → null). Created `BudgetLimitSuggester` scanning 90-day transaction history per category with trimmed mean (drops top/bottom 10%) and publishing `BudgetLimitSuggested`. Budget creation sheet now shows a `SuggestionChip` with the suggested amount when a category is selected. Started `RuleLearner` in `SciuroApp.onCreate()` alongside the orchestrator.
### Changed
