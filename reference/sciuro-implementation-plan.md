...
- **`IngestionSource` interface**: `NotificationSourceAdapter` is fully built in v4 (covers banks + e-wallets, Section 8). `EmailSourceAdapter` and `SMSSourceAdapter` are **scaffolded** — interface defined, not implemented — so adding a real source later is a matter of writing one adapter class, not redesigning the pipeline. This is deliberately scoped down from "build everything now" to keep v4 shippable, while directly answering "sources we haven't foreseen yet" with an actual extension point rather than a promise.
