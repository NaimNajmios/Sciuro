...
* **Learned Merchant Rules:** Sciuro's automation system learns merchant→category mappings from manual corrections and surfaces them in a dedicated settings screen. Users can view all learned rules with trust status (based on confirmation count), reset incorrectly learned mappings, and override the category assignment — providing transparency and control over the classification system's internal state. A separate Accounts tab shows learned merchant→account associations, which the system uses to auto-assign transactions to accounts.
* **Investment Live Pricing:** Individual investment cards display both book value and live price from Yahoo Finance with green/red color coding. Pull-to-refresh updates live prices. Manual price overrides can be set per investment from Settings → Data & Privacy → Investment Price Overrides.
* **BNPL Risk Indicator:** Debt Overview screen shows an amber warning card when 2+ active BNPL or credit card debts are detected, with count and total amount.
* **Credit Card Statements:** Debt detail sheets for credit card debts show the current statement balance, minimum payment due, payment count for the cycle, and days remaining.
* **Income Pattern Display:** Dashboard runway card shows the next expected income amount and date when a recurring income pattern is detected, replacing the generic "based on bills only" label.
* **Weekly Digest Card:** Dashboard displays a real-time summary of the last 7 days' spending — total, transaction count, and top category.
* **Net Worth Milestones:** Dashboard hero panel shows the highest net worth milestone reached (e.g., "RM 10K milestone reached") as a persistent badge.
* **Obligation Amount Drift:** Bill cards in Kanban display an amber "AMOUNT CHANGED" badge when a recurring bill's amount deviates >20%, with old→new amounts shown in the detail sheet.
* **Transfer Candidate Review:** Unmatched transfer candidates in the Kanban Review tab are labeled with a "TRANSFER" badge, distinguishing them from regular unreviewed transactions.
...
* **Audit-First Architecture:** Every data mutation passes through a unified Audit Log, ensuring complete traceability. Each mutation and its audit entry commit atomically inside the same database transaction — a crash can never leave a committed-but-unaudited change. The Developer Options → Health tab exposes an Audit Integrity counter reporting any transaction that lacks a matching audit record.
...
* **Durable Domain Event Delivery:** All 27 domain event types are persisted durably before live emission via a SQLDelight-backed event log (`domain_event_log`) with per-subscriber delivery state (`domain_event_delivery`). Slow or temporarily absent subscribers miss no events — historical events are replayed on startup, expired leases trigger retries, and repeated failures enter a dead-letter state. Developer Health tab surfaces pending deliveries, dead letters, retries, live drops, and subscriber lag with color-coded indicators. Critical events (`DebtFullyPaidOff`, `NetPositionMilestoneReached`) receive the same durable delivery guarantee.

## Project Status

The project is fully functional and has completed **Phase T1 (Atomic Audit Writes & Audit Integrity)**, **Phase G4 (Domain Event Reliability)**, **Phase K3 (Scroll Fix & Dashboard Transaction Display)**, **Phase K2 (UX Polish — Complete)**, **Phase K1 (UI Polish & Accessibility Pass)**, **Phase R2 (Engine Trigger Persistence & Concurrency Hardening)**, **Phase R1 (Reliability & Performance Hardening)**, **Phase N2 (User-Configurable Notification Preferences)**, **Phase J2 (Developer Options & Settings Redesign)**, **Phase S1 (Database Safety — Key-Loss Detection & Quarantine)**, **Phase S2 (Kanban/Debt/Obligation Edge Case Fixes)**, **Core-Classifier Edge-Case Hardening (Phases 1-4)**, **Pipeline Edge-Case Hardening**, **M1 — Merchant Rules UI (Dark Component Gap Closure)**, and **Dev-Tools-Pagination-Hero-Palette** (ingestion log pagination + mark-as-read, pipeline trace pagination, HeroPanel palette-aware colors) addressing cross-cutting architectural concerns.
...
- **Locale-independent merchant matching:** All `lowercase()` calls in `CategoryResolver`, `ReviewTierDecider`, `RuleLearner`, and `AccountMatcher` use `lowercaseChar()` — Turkish/Azeri locale no longer breaks merchant string comparisons.
- **Atomic re-processing guard:** `markProcessing` SQL updated to `WHERE status = 'PENDING'` with a Kotlin status check inside the dedup mutex — duplicate event IDs can no longer produce double-booked transactions.
- **Dedup by merchant:** `findLikelyDuplicate` now matches on amount + direction + timestamp + merchant, reducing false positives for same-amount same-time transactions to different merchants.
- **Parser failure retry:** Null parser result no longer permanently dead-letters the event; it stays PENDING and is retried every 60s via periodic `recoverStrandedEvents`.
- **Max source reconnect attempts:** `collectEventsWithRetry` exits after 10 consecutive failures instead of retrying forever.
- **Account deletion cascade:** Deleting an account clears associated `merchant_account_rule` rows inside a DB transaction.
- **Confidence threshold centralized:** `isHighConfidence` extracted once and reused for `parseStage`, `extractionMethod`, and `auditSource` — eliminating three duplicated checks.
...
**Phase T1 (Atomic Audit Writes & Audit Integrity) highlights:**
- **Atomic mutation + audit:** Every SQLDelight-backed mutation path now wraps the data change and its audit entry in a single `database.transaction { }` (or `transactionWithResult { }` for value-returning mutations). If the audit write throws, the entire mutation rolls back — no change is ever committed unaudited. This closes the crash-window where `withAudit` wrote the mutation first and the audit as a separate statement.
- **Synchronous audit writes:** `AuditRepository.logMutation()` is now synchronous, so it can run inside SQLDelight's non-suspending transaction lambda (previously blocked the pattern in KMP `commonMain`).
- **Coverage everywhere:** Applied to `TransactionRepository` (book/review/approve/reject/delete/edit/updateCategory/undoAutoConfirm), `CashAdjustmentRepository`, `AccountRepository`, `CategoryRepository` (update/delete were previously **not audited at all** — now covered), `ReconciliationEngine`, `ObligationRepository`, `DebtRepository`, `InvestmentRepository`, `BudgetRepository`, and `TransferRepository`. The non-atomic `withAudit` wrapper was removed.
- **Balance updates inside the transaction:** Cash adjustment and reconciliation paths call `database.accountQueries.updateBalance()` directly inside the transaction instead of `AccountRepository.updateBalance()` (which carried its own nested audit wrapper).
- **Audit integrity check:** New `auditIntegrityCheck` query (`SELECT COUNT(*) FROM transaction_record WHERE id NOT IN (SELECT entityId FROM audit_log_entity WHERE entityType = 'TRANSACTION')`) exposed as `AuditRepository.getAuditIntegrityGaps()` and shown on the Developer Options → Health tab (green at zero, danger-red when gaps exist).
- **`AuditAtomicityTest`:** 4 integration tests verifying atomic commit, audit-failure rollback (transaction row AND balance reverted), orphan-transaction detection, and clean-state zero-gap.
...
**Phase K2 (Phase 1 — Tactile Polish) highlights:**
- **Unified haptic architecture:** `SciuroHaptics` utility with semantic methods (`selection`, `success`, `warning`, `error`, `transferMatch`). All 13+ scattered raw `performHapticFeedback` calls replaced. Uses Compose `LocalHapticFeedback` — no `VIBRATE` permission needed. Numpad keypad uses light `selection` tick, primary buttons use `success` tap, cards use `selection` instead of `LongPress`.
- **Long-press context menus:** Transaction, budget, and debt cards now show contextual `DropdownMenu` on long-press — "Copy amount" (clipboard), "Mark as transfer", "Edit budget", "Delete debt", etc. Uses `combinedClickable` via extended `bounceClick` modifier.
- **Scroll-aware status bar:** Dashboard status bar transitions from transparent/primary (HeroPanel visible) to surface color when scrolled. Status bar icon color adapts automatically. Uses `derivedStateOf` for efficient recomposition.
- **Dynamic HeroPanel top padding:** Replaced fixed 48dp with `WindowInsets.statusBars` + 24dp for correct status bar clearance across devices.

**Phase K2 (Phase 2 — Loading & Navigation) highlights:**
- **Shimmer skeleton loading:** New `SciuroSkeleton` with reusable `shimmerEffect()` modifier (animated gradient). `DashboardSkeleton` shown during initial load (hero placeholder + 6 skeleton rows). Respects `reducedMotion()`. Zero new dependencies.
- **Predictive back gesture:** `OnBackInvokedCallback` registered on API 34+ for system-level predictive back animation (manifest flag already set).
- **Material You Dynamic Color:** New `PalettePreference.DYNAMIC` option. On API 31+, colors derive from device wallpaper via `dynamicLightColorScheme`/`dynamicDarkColorScheme`. No new dependencies — already in existing `material3` artifact. Option hidden below API 31.

**Phase K2 (Phase 3 — Security & Reach) highlights:**
- **Biometric confirmation for destructive actions:** New `BiometricConfirmDialog` composable gating delete transaction, delete budget, and delete debt with biometric/PIN re-auth. Falls back to direct execution when no credential is enrolled. Reuses existing `BIOMETRIC_STRONG | DEVICE_CREDENTIAL` pattern from `BiometricGate`.
- **Inline notification actions:** Transfer review notifications now have "Yes, it's me" / "Not me" action buttons. Bill reminders have "Mark as paid" / "Snooze 1 day". Actions handled by new `NotificationActionReceiver` BroadcastReceiver.

**Phase K2 (Phase 4 — Intelligence & Widgets) highlights:**
- **Predictive runway analytics:** Dashboard shows "Projected Runway: X days" with color-coded sparkline (red <7d, yellow <14d, green >14d), confidence percentage, and daily burn rate. Prediction uses 30-day historical spending + upcoming obligations.
- **Per-account spending velocity:** Account Detail shows daily average spend, burn rate progress bar (color-coded at 50%/80% thresholds), days until depleted, and trend arrow (up/down vs. previous month).
- **Reusable Sparkline component:** New `Sparkline` composable in core-ui — configurable Canvas sparkline used by PredictionCard. Also usable by other cards.
- **Glance home screen widgets:** New `:feature-widget` module. Balance widget (net position) and Budget widget (top 3 at-risk budgets). Auto-refresh every 30 minutes. No tap actions (widget module can't depend on app module at compile time).
...
* **Core Modules** (`core-*`): Reusable domain layers and intelligence engines:
  - `:core-ledger`, `:core-audit`: Foundational persistence and traceability. `core-audit` defines the durable `DomainEventStore` interface, `DomainEventBus` with `metrics()` and `subscribe()` APIs, `DomainEventCodec` for serialization, and `DomainEventEnvelope` for the immutable event log. `core-ledger` implements `SqlDelightEventStore` backed by two SQLDelight tables (`domain_event_log` for the immutable event log, `domain_event_delivery` for per-subscriber delivery state) with schema migration 15.sqm. The bus persists every event durably before live emission, ensuring no event is silently lost regardless of subscriber state.
...
`HeroPanel` (`core-ui`) is a hero header composable used across 8 screens. Its background dynamically inherits `MaterialTheme.colorScheme.primary` — reflecting the user's selected palette (Monochrome, Amber, Ocean, Forest, Plum, or Slate) instead of a fixed black. All text, icons, and the `WaveChart` sparkline use `MaterialTheme.colorScheme.onPrimary` for contrast. The `PillToggle` inside the hero computes `isOnDarkSurface` from the primary color's luminance. The top row uses `SpaceBetween` to place the title on the left and an optional `PillToggle` on the right. A second row renders the large hero figure at full width below. An optional `WaveChart` sparkline and a `content` slot complete the section:
...
| **Ingestion Log** | Dead-letter event viewer with paginated loading (50-items per page, "Load More" at the bottom). Events can be marked as read individually or in bulk ("Mark All Read" button). Unread events shown by default; a "Show read" toggle reveals previously acknowledged items. Summary card shows pending/dead-letter counts with per-event error display and manual refresh. Tapping a dead letter opens a form to structurally edit the raw JSON payload and requeue it. |
...
| **Health** | Per-package parser match-rate monitoring in a `SciuroCard` with summary row (packages, processed, average match rate). Pipeline metrics card with LLM fallback counts and dead-letter counts. Domain Event Bus metrics card with color-coded indicators for pending deliveries, dead letters, retries, live drops, oldest pending age, and subscriber lag. Audit Integrity card showing the count of transactions missing an audit record (green at zero, danger-red when gaps exist). |
| **Pipeline Trace** | Trace event viewer with paginated loading (default 100, "Load More" increases by 100 per tap). Filter changes reset the limit. Stage-by-stage breakdown (SUCCESS/FAILURE/DROP), duration, confidence, and detail JSON. Filter section (package, outcome, allowlisted toggle) wrapped in `SciuroCard`. Sessions prominently display their package name (e.g. `com.whatsapp`) and capture early drops. Detail cards use `RoundedCornerShape(12.dp)`. |
