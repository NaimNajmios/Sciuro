...
| **Health** | Per-package parser match-rate monitoring in a `SciuroCard` with summary row (packages, processed, average match rate). Pipeline metrics card with LLM fallback counts and dead-letter counts. Domain Event Bus metrics card with color-coded indicators for pending deliveries, dead letters, retries, live drops, oldest pending age, and subscriber lag. |
